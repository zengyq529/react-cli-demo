...
import { fetchList, fetchListCallback, fetchList404, fetchList2 } from './fetchdemo';
...
describe('分组 简单', () => {
  ...
  test('对象对比toBeNull', () => {
    expect(null).toBeNull(); //对象不能用boBe 引用地址不一样。
  });
  test('对象对比toBeUndefined', () => {
    expect(undefined).toBeUndefined(); //对象不能用boBe 引用地址不一样。
  });
  test('对象对比toBeDefined', () => {
    const a = null;
    expect(a).toBeDefined(); //对象不能用boBe 引用地址不一样。
  });
  test('对象对比toBeTruthy/toBeFalsy', () => {
    const a = 1 // true;
    expect(a).toBeTruthy(); //对象不能用boBe 引用地址不一样。
    expect(!a).toBeFalsy(); //对象不能用boBe 引用地址不一样。
  });
...
describe('分组 异步', () => {
  ...
  //   return expect(fetchList2()).resolves.toMatchObject({
  //     "kind": "Listing"
  //   })
  // })
  test('异步测试用例3 async', async () => { // done :fun 调用了done函数才认为执行结束。
    await expect(fetchList()).resolves.toHaveProperty('kind');
  })
  ...
  test('异步测试用例404', () => { // done :fun 调用了done函数才认为执行结束。
    ...
    return fetchList404().catch((e) => {
      expect(e.toString().indexOf('443') > -1).toBe(true)
    })
  })
})
